...
contract CalcFee {
    ...
    // Guide for calling:
    function calculate_fee(uint256 new_k, uint256 old_k, uint256 platform_fee, uint256 circulating_shares) public pure returns (uint256 sharesToMint) {
        if (new_k > 2**128 -1) { return 0; }
        if (old_k > 2**128 -1) { return 0; }
        if (platform_fee > FEE_ACCURACY) { return 0; }
        ...
        uint256 _scaledTargetOwnership = _scaledMultiplier * platform_fee / FEE_ACCURACY;   // Assert: < uint144 during maths, ends < uint128
        ...
        // than the smaller of the two numbers. So if token_a has 18 decimals and token_b has 10 decimals we can expect circulating_shares / liquidity
    ...
    function sqrt_calculate_fee(uint256 new_k, uint256 old_k, uint256 platform_fee, uint256 circulating_shares) public pure returns (uint256 sharesToMint) {
        if (platform_fee > FEE_ACCURACY) { return 0; }
        if (circulating_shares > 2**128 -1) { return 0; }
