...
contract RouterEventEmitter {
    ...
    function swapExactTokensForTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IVexchangeV2Router01(router).swapExactTokensForTokens.selector, amountIn, amountOutMin, path, to, deadline
        ));
        assert(success);
    ...
    function swapTokensForExactTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IVexchangeV2Router01(router).swapTokensForExactTokens.selector, amountOut, amountInMax, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
